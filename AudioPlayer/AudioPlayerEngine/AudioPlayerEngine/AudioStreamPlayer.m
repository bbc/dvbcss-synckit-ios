...
//------------------------------------------------------------------------------
#pragma mark - constants declarations
//------------------------------------------------------------------------------
...
NSString * const AS_AUDIO_SESSION_SETUP_FAILED_STRING = @"Audio session setup failed";

//------------------------------------------------------------------------------
#pragma mark - AudioStream interface extension
//------------------------------------------------------------------------------

@interface AudioStreamPlayer ()

/**
 *  Properties permission redefinition
 */
@property (readwrite) AudioStreamPlayerState state;

/**
 *  State before last change
 */
@property (readwrite) AudioStreamPlayerState laststate;


//------------------------------------------------------------------------------
#pragma mark - handlers for audio callback functions
//------------------------------------------------------------------------------

/**
 *  Object method which handles implementation of ASPropertyListenerProc
 *
 *  @param inAudioFileStream should be the same as self->audioFileStream
 *  @param inPropertyID      the property that changed
 *  @param ioFlags           the ioFlags passed in
 */
 ...
 *
 *  @param inInputData          the packet data
 *  @param inNumberBytes        byte size of the data
 *  @param inNumberPackets      number of packets in the data
 *  @param inPacketDescriptions packet descriptions
 */
 ...
 *
 *  @param inAQ     the queue
 *  @param inBuffer the buffer
 */
 ...
 *
 *  @param inAQ the audio queue
 *  @param inID the property ID
 */
...
#if TARGET_OS_IPHONE
/**
 *  Handler for Audio Session interrution notification
 *
 *  @param notification an AVAudioSessionInterruptionNotification object
 */
...
/**
 *  Internal method to seek to a new position. Called from our internal runloop to reopen the stream at a seeked location
 *
 *  @param newSeekTime new seek position in seconds
 */
 ...
 *
 *  @param aStream   the network file stream
 *  @param eventType the event which triggered this method
 */
 ...
 *   Invoked by an audio file stream parser when it finds a property value in the audio file stream.
 ...
 ready to begin playback (playback won't beginuntil audio packets are
 ...
 *
 *  @param inClientData      pointer to structure returned in callback
 *  @param inAudioFileStream The ID of the audio file stream parser that invoked the callback.
 *  @param inPropertyID      The four-character ID of the property that the parser found in the audio file data stream
 *  @param ioFlags           See @link https://developer.apple.com/reference/audiotoolbox/1669809-audio_file_stream_services/audio_file_stream_flags?language=objc  Audio File Stream flags @/link
 */
                                   ...
                                   UInt32 *						ioFlags)
{
	...
	AudioStreamPlayer* streamer = (__bridge AudioStreamPlayer *)inClientData;
	[streamer
...
/**
 *  @brief Invoked by an audio file stream parser when it finds audio data in the audio file stream.
 *
 *  @discussion When the AudioStream has packets to be played, this function gets an
 ...
 *  CBR functionality added.
 *
 *  @param inClientData         The value you provided in the inClientData parameter when the AudioFileStreamOpen function was called.
 *  @param inNumberBytes         number of bytes of data in the inInputData buffer.
 *  @param inNumberPackets       number of packets of audio data in the inInputData buffer
 *  @param inInputData          The audio data.
 *  @param inPacketDescriptions An array of audio file stream packet description structures describing the data.
 */
 ...
 *
 *  @param inClientData The value you provided in the inClientData parameter when the AudioFileStreamOpen function was called.
 *  @param inAQ         audio queue object
 *  @param inBuffer     audio queue buffers
 */
...
{
	AudioStreamPlayer* streamer = (__bridge AudioStreamPlayer *)inUserData;
...
{
	AudioStreamPlayer* streamer = (__bridge AudioStreamPlayer *)inClientInfo;
	[streamer handleReadFromStream:aStream eventType:eventType];
}


//------------------------------------------------------------------------------
#pragma mark AudioStreamPlayer Implementation
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
#pragma mark Initialisers, Lifecycle methods
//------------------------------------------------------------------------------
/**
...
// isWaiting
//
// returns YES if the AudioStreamPlayer is waiting for a state transition of some
// kind.
//
- (BOOL)isWaiting
{
...
//
- (void)setState:(AudioStreamPlayerState)aStatus
{
    @synchronized(self)
    {
        if (state != aStatus)
...
- (void)failWithErrorCode:(AudioStreamPlayerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		if (err)
		{
			...
			NSLog(@"%@ err: %c%c%c%c %d\n",
                  [AudioStreamPlayer stringForErrorCode:anErrorCode],
                  errChars[3], errChars[2], errChars[1], errChars[0],
                  (int)err);
		}
		else
		{
			NSLog(@"%@", [AudioStreamPlayer stringForErrorCode:anErrorCode]);
		}
...
//------------------------------------------------------------------------------
#pragma mark Open audio session, read audio stream data
//------------------------------------------------------------------------------
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		CFHTTPMessageRef message= CFHTTPMessageCreateRequest(NULL, requestMethod, (__bridge CFURLRef)url, kCFHTTPVersion1_1);
        ...
        resultOK = CFReadStreamSetProperty(
                                         stream,
...
//                            );
    ...
    [[NSNotificationCenter defaultCenter]   addObserver:self
                                            selector:@selector(handleInterruptionChangeToState:)
...
//                             );
    ...
    {
        [self failWithErrorCode:AS_AUDIO_SESSION_SETUP_FAILED];
        
    }
    
    nsError = nil;
    
    [[AVAudioSession sharedInstance] setActive:YES error:&nsError];
...
//	- Creation of the AudioStreamPlayer (other, likely "main" thread)
                   ...
                   eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
            ...
            [AudioStreamPlayer hintForFileExtension:self.fileExtension];
            
			// create an audio file stream parser
			err = AudioFileStreamOpen((__bridge void *)(self)
