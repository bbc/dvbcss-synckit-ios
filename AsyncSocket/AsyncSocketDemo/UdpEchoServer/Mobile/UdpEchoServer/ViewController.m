...
//			return;
//		}
        NSDictionary *interfaces = [ViewController availableNetworkInterfaces];
        NSData *sourceAddress = nil;
        if( !sourceAddress ) sourceAddress = [[interfaces allValues] firstObject];
        
//        if(![udpSocket bindToAddress:sourceAddress error:&error]) {
//            [self logError:FORMAT(@"Error starting server (bind): %@", error)];
//            return;
//        }
        ...
        if(![udpSocket joinMulticastGroup:SSDPMulticastGroupAddress error:&error]) {
            [self logError:FORMAT(@"Error starting server (joinMulticastGroup): %@", error)];
            return;
        }
